...
{
    echo 'Docker build...'
    sudo docker build -t provide/providibright .

    echo 'Docker tag...'
    sudo docker tag provide/providibright:latest "${AWS_ACCOUNT_ID}.dkr.ecr.us-east-1.amazonaws.com/provide/providibright:${buildRef}"

    echo 'Docker push...'
    $(aws ecr get-login --no-include-email --region us-east-1)
    sudo docker push "${AWS_ACCOUNT_ID}.dkr.ecr.us-east-1.amazonaws.com/provide/providibright:${buildRef}"
}
...
ecs_deploy()
{
    ...
    echo 'Listing images...'
    ECR_IMAGE_DIGEST=$(aws ecr list-images --repository-name provide/providibright | jq '.imageIds[0].imageDigest')
    ...
    sed -E "s/providibright:[a-zA-Z0-9\.-]+/providibright:${buildRef}/g" "./${DEFINITION_FILE}" > "./${MUNGED_FILE}"
