...
if [[ -z "${DATABASE_NAME}" ]]; then
  export DATABASE_NAME=axiom_dev
fi

if [[ -z "${DATABASE_USER}" ]]; then
  export DATABASE_USER=axiom
fi

if [[ -z "${DATABASE_PASSWORD}" ]]; then
  export DATABASE_PASSWORD=axiom
fi
...
export VAULT_SEAL_UNSEAL_KEY='traffic charge swing glimpse will citizen push mutual embrace volcano siege axiom-apiify gossip battle casual exit enrich unlock muscle vast female initial please day'
